  ...
  const sendMessage = useCallback(
    async (userMessage: string) => {
      ...
      try {
        ...
        const newTurnsLeft = Math.max(
          0,
          MAX_CONTEXT_TURNS - Math.floor((newHistory.length + 1) / 2)
        );
        ...
        const botDisplayMsg: DisplayMessage = {
          ...
          type: 'bot',
          timestamp: getCurrentTime(),
          ...(isError ? {} : { mode: currentMode, turnsLeft: newTurnsLeft }),
        };
        setDisplayMessages((prev: DisplayMessage[]) => [...prev, botDisplayMsg]);
        setTypingMessageId(botMsgId);
        setIsTyping(true);

        // Add bot response to conversation history only if it's a real reply
        if (!isError) {
