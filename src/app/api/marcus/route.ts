...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`[Marcus API] System instruction length: ${systemInstruction?.length || 0} chars`);
      return NextResponse.json(
        { reply: 'The philosophical networks encountered an error. The mode instruction may be too complex.', isError: true },
        { status: 500 }
      );
    }
    ...
    if (!reply) {
      console.log(`[Marcus API] No reply from API`);
      return NextResponse.json({ reply: 'I find myself in contemplative silence...', isError: true });
    }

    console.log(`[Marcus API] Response received successfully`);

    return NextResponse.json({ reply, isError: false });
  } catch (error) {
    console.error('[Marcus API] Error:', error);
    return NextResponse.json(
      { reply: 'The philosophical networks seem clouded at the moment...', isError: true },
      { status: 500 }
    );
  }
}
